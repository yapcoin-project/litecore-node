...
var Bcoin = require('./bcoin'); // !!!: DO not change this to ycoin
var BcoinTx = require('ycoin').tx;
var Networks = require('yapcore-lib').Networks;
var LRU = require('lru-cache');
...
P2P.prototype._startBcoin = function(callback) {
  ...
  if (['livenet', 'live', 'main', 'mainnet'].indexOf(this.node.network) !== -1) {
    network = 'main';
    port = this._configPeers[0].port || 7333;
  } else if (this.node.network !== 'regtest') {
    network = 'testnet';
    port = this._configPeers[0].port || 17332;
  } else {
    network = this.node.network;
    port = this._configPeers[0].port || 17338;
  }
