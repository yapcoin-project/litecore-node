...
var BASE_PACKAGE = {
  description: 'A full Yapcoin node build with Yapcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'yapcore-lib': '^' + bitcore.version,
    'yapcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/yapcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the yapcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the yapcore-node directory and configuration
