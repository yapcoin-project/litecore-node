...
/**
 * Will return the path and default yapcore-node configuration. It will search for the
 * configuration file in the "~/.yapcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.yapcore');
  var defaultConfigFile = path.resolve(defaultPath, './yapcore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of Yapcore.`);

    var now = new Date();
    // bitcore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `yapcore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
