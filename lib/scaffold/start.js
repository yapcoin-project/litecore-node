...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './yapcore-node.json');
...
function loadModule(req, service) {
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - yapcoreNode field. All paths failed to find valid nodeJS code.');
  }
